...
{
    wifi_util_info_print(WIFI_DMCLI, "%s:%d get_total_num_apmld_dml called\n", __func__, __LINE__);
    ...
    for (int i = 0; i < count; i++) {
        wifi_util_info_print(WIFI_DMCLI, "%s:%d MLD_ID[%d] = %u\n", __func__, __LINE__, i, (unsigned char)mld_id_map[i]);
    }
